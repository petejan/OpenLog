...
byte appendFile(char* fileName)
{
  ...
  const int LOCAL_BUFF_SIZE = 64; //This is the 2nd buffer. It pulls from the larger Serial buffer as quickly as possible.
  ...
  while (1)
  {
    size_t charsToRecord = NewSerial.read(buff, sizeof(buff));
    if (charsToRecord > 0)
    {
#if DEBUG>0
  	  NewSerial.print("c ");
  	  NewSerial.println(charsToRecord);
#endif

  	  toggleLED(stat1); //Toggle the STAT1 LED each time we record the buffer
      for (byte i =0;i<charsToRecord;i++)
      {
#if DEBUG>0
    	  NewSerial.print("i ");
    	  NewSerial.print(i);
    	  NewSerial.print(' ');
    	  NewSerial.println(buff[i]);
#endif

    	  if (ts_arm && (buff[i] >= ' '))
    	  {
    		  workingFile.print(millis());
    		  workingFile.print(' ');
    	  ...
    	  {
#if DEBUG>0
    		  NewSerial.println("ts arm");
#endif
    		  ts_arm = true;
    	  }

    	  workingFile.write(buff[i]);
      }
